...
def parse():
    ...
    group = parser.add_argument_group(
        "Specific arguments used by the sentencepiece token type"
    )
    group.add_argument(
        "--model_type",
        type=str,
...
def generate_vocab_char(
    ...
    save_token_vocab(
        ...
        text2tokens_func=text2char_list,
        vocab_size=vocab_size,
        save_idx2text_len=True,
    )


def generate_vocab_sentencepiece(
    save_path: str,
    text_path: str,
    txt_format: str,
    vocab_size: int,
    ...
    save_path = os.path.join(
        save_path, f"{model_type}{get_readable_number(vocab_size)}", txt_format
    )
    os.makedirs(save_path, exist_ok=True)
    ...
    if not os.path.exists(os.path.join(save_path, "model")) or not os.path.exists(
        os.path.join(save_path, "vocab")
    ):
        ...
        print(
            f"SentencePiece tokenizer model has been successfully saved to {model_path}."
        )
        ...
        token_vocab = np.loadtxt(vocab_path, dtype=str, ndmin=2)[:, 0].tolist() + [
            "<sos/eos>"
        ]
    ...
    if not os.path.exists(idx2text_len_path):
        ...
        np.savetxt(
            idx2text_len_path,
            [
                [idx, len(sp_tokenizer.text2tensor(text))]
                for idx, text in idx2text.items()
            ],
            fmt="%s",
        )
        print(
            f"The length of SentencePiece tokenized text has been successfully saved to {idx2text_len_path}."
        )


def generate_vocab_g2p(
    save_path: str, text_path: str, txt_format: str, vocab_size: str
):
    assert vocab_size in [
        "stress",
        "no-stress",
    ], "vocab_size for g2p should be one of ['stress', 'no-stress']!"
    g2p = G2p()
    ...
    def text2phn_list(x: str):
        ...
        for phn in _phn_list:
            ...
            # punctuation marks
                ...
                # (phn_list may be empty when the sentence starts with a punctuation mark)
                if phn_list and phn_list[-1] == "<space>":
                    phn_list[-1] = phn
                else:
                    phn_list.append(phn)
    ...
    save_token_vocab(
        ...
        text_path=text_path,
        txt_format=txt_format,
        text2tokens_func=text2phn_list,
        vocab_size=vocab_size,
        save_idx2text=True,
        save_idx2text_len=True,
    )
...
def main(
    ...
    # unknown token branch
        ...
        raise ValueError(
            f"token_type must be one of ['char', 'sentencepiece', 'g2p', 'word'], but got {token_type}!"
        )
