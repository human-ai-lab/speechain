...
def dump_subset_metadata(
    ...
    subset_indices = list(
        load_idx2data_file(os.path.join(dataset_path, subset_name, "idx2wav")).keys()
    )
    subset_idx2text = {
        index: idx2text[index] for index in subset_indices if index in idx2text.keys()
    }
    subset_idx2duration = {
        index: idx2duration[index]
        for index in subset_indices
        if index in idx2duration.keys()
    }
    subset_path = os.path.join(
        save_path,
        subset_name,
        "stress" if retain_stress else "no-stress",
    ...
    np.savetxt(
        text_path, [[idx, str(text)] for idx, text in subset_idx2text.items()], fmt="%s"
    )
    ...
    np.savetxt(
        text_len_path,
        [[idx, len(text)] for idx, text in subset_idx2text.items()],
        fmt="%s",
    )
    print(
    ...
    np.savetxt(
        duration_path,
        [[idx, str(duration)] for idx, duration in subset_idx2duration.items()],
        fmt="%s",
    )
    print(
        f"The duration of tokenized text has been successfully saved to {duration_path}."
    )


def cal_duration_by_tg(
    tg_file_list: List[str], idx2punc_text: Dict[str, str], retain_stress: bool
):
    """
    ...
    for tg_file in tqdm(tg_file_list):
        ...
        word_boundary_dict = {
            round(word.maxTime, 2): True if word.mark != "" else False
        ...
        for phn in text_grid.tiers[1].intervals:
            ...
            phn_duration, phn_boundary = round(phn.maxTime - phn.minTime, 2), round(
                phn.maxTime, 2
            )
            ...
            # for the silence tokens
                ...
                if (
                    len(idx2text[file_name]) == 0
                    or idx2text[file_name][-1] != "<space>"
                ):
            ...
            if (
                phn_boundary in word_boundary_dict.keys()
                and word_boundary_dict[phn_boundary]
            ):
                idx2text[file_name].append("<space>")
        ...
        # add punctuation marks in phoneme sequence with 0 duration
            ...
            if punc_text is not None:
                ...
                for word in word_list:
                    ...
                    try:
                        if idx2text[file_name][
                            curr_phn_idx
                        ] == "<space>" and curr_phn_idx < len(idx2text[file_name]):
                            _tmp_text.append(idx2text[file_name][curr_phn_idx])
                            _tmp_duration.append(idx2duration[file_name][curr_phn_idx])
                            curr_phn_idx += 1
                    except IndexError:
...
def combine_subsets(subset_path_list: List[str], new_subset_name: str, save_path: str):
    ...
    # defaultdict is used so that any 'idx2*' metadata file other than the three common ones
    # (e.g., 'idx2spk') can also be merged instead of raising a KeyError.
    meta_data_dict = {
        first_level_folder: {
            second_level_folder: defaultdict(
                dict, {"idx2duration": {}, "idx2text": {}, "idx2text_len": {}}
            )
            for second_level_folder in get_folder_intersection(
                [
                    os.path.join(subset_path, first_level_folder)
    ...
    for first_level_folder in meta_data_dict.keys():
        for second_level_folder in meta_data_dict[first_level_folder].keys():
            for subset_path in subset_path_list:
                curr_metadata_path = os.path.join(
                    subset_path, first_level_folder, second_level_folder
                )

                for metadata in os.listdir(curr_metadata_path):
                    if not metadata.startswith("idx2"):
                        continue
                    meta_data_dict[first_level_folder][second_level_folder][
                        metadata
                    ].update(
                        load_idx2data_file(os.path.join(curr_metadata_path, metadata))
                    )

            # Calculate the phoneme vocabulary
            subset_phns = []
            for text in meta_data_dict[first_level_folder][second_level_folder][
                "idx2text"
            ].values():
                subset_phns += [phoneme[1:-1] for phoneme in text[1:-1].split(", ")]
            meta_data_dict[first_level_folder][second_level_folder]["vocab"] = (
                convert_phns_to_vocab(subset_phns)
            )
    ...
    for first_level_folder in meta_data_dict.keys():
        for second_level_folder in meta_data_dict[first_level_folder].keys():
            folder_path = os.path.join(
                new_subset_path, first_level_folder, second_level_folder
            )
            os.makedirs(folder_path, exist_ok=True)

            for metadata in meta_data_dict[first_level_folder][
                second_level_folder
            ].keys():
                metadata_path = os.path.join(folder_path, metadata)

                if isinstance(
                    meta_data_dict[first_level_folder][second_level_folder][metadata],
                    Dict,
                ):
                    np.savetxt(
                        metadata_path,
                        [
                            [idx, str(m_d)]
                            for idx, m_d in meta_data_dict[first_level_folder][
                                second_level_folder
                            ][metadata].items()
                        ],
                        fmt="%s",
...
def main(
    ...
    retain_punc: bool = False,
    retain_stress: bool = True,
    save_path: str = None,
    ncpu: int = 8,
):
    """
    ...
    tg_file_list = search_file_in_subfolder(
        textgrid_path, tgt_match_fn=lambda x: x.endswith(".TextGrid")
    )
    if len(tg_file_list) == 0:
        raise RuntimeError(
            f".TextGrid files have not been successfully saved to {textgrid_path}!"
        )
    ...
    if retain_punc:
        ...
        idx2punc_text = load_idx2data_file(
            search_file_in_subfolder(
                _search_path, tgt_match_fn=lambda x: x == "idx2punc_text"
            )
        )
    ...
    cal_duration_by_tg_func = partial(
        cal_duration_by_tg, idx2punc_text=idx2punc_text, retain_stress=retain_stress
    )
    ...
    if subset_name is None:
        for file_name in os.listdir(textgrid_path):
            ...
            if os.path.isfile(os.path.join(textgrid_path, file_name)):
                continue
            dump_subset_metadata(
                dataset_path=dataset_path,
                save_path=save_path,
                subset_name=file_name,
    ...
    if dataset_name == "libritts":
        ...
        if os.path.exists(clean_100_path) and os.path.exists(clean_360_path):
            combine_subsets(
                subset_path_list=[clean_100_path, clean_360_path],
                new_subset_name="train-clean-460",
                save_path=save_path,
            )
            ...
            if os.path.exists(clean_460_path) and os.path.exists(other_500_path):
                combine_subsets(
                    subset_path_list=[clean_460_path, other_500_path],
                    new_subset_name="train-960",
                    save_path=save_path,
                )
        ...
        if os.path.exists(dev_clean_path) and os.path.exists(dev_other_path):
            combine_subsets(
                subset_path_list=[dev_clean_path, dev_other_path],
                new_subset_name="dev",
                save_path=save_path,
            )
