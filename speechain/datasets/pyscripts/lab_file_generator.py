...
def parse():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to overwrite the existing .lab files. If False, the existing .lab files "
        "are kept and only the missing ones are generated. (default: False)",
    )
    parser.add_argument(
        "--ncpu",
        type=int,
...
def save_lab_files(idx2wav_text: List[List[str]], lab_cover_flag: bool = False):
    for idx, wav_path, text in tqdm(idx2wav_text):
        ...
        # only skip the current file instead of aborting the whole chunk
...
def proc_subset(idx2wav_path: str, lab_cover_flag: bool = False, ncpu: int = 8):
    ...
    if not os.path.exists(idx2text_path):
        raise RuntimeError(
            f"idx2no-punc_text doesn't exist in {subset_path}! "
...
def main(corpus_path: str, lab_cover_flag: bool = False, ncpu: int = 8):
    ...
    if os.path.exists(os.path.join(corpus_path, "idx2wav")):
        proc_subset(
            idx2wav_path=os.path.join(corpus_path, "idx2wav"),
    ...
    # corpus_path is the folder path of a dataset (including many subsets as sub-folders)
        ...
        for subset in os.listdir(corpus_path):
            ...
            if os.path.exists(idx2wav_path):
                proc_subset(
                    idx2wav_path=idx2wav_path, lab_cover_flag=lab_cover_flag, ncpu=ncpu
                )
            else:
                raise RuntimeError(
                    f"idx2wav doesn't exist in {os.path.join(corpus_path, subset)}!"
                )
