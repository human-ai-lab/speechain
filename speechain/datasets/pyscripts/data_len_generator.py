...
def get_data_length(idx2data: List[List[str]]):
    ...
    return [
        [idx, read_data_by_path(data_path).shape[0]]
        for idx, data_path in tqdm(idx2data, desc="Processing data files")
    ]
...
def main(src_file: str, ncpu: int = 8):
    ...
    if not os.path.exists(tgt_path):
        ...
        data_length_list = [
            item for sublist in data_length_list_nproc for item in sublist
        ]
        np.savetxt(tgt_path, sorted(data_length_list, key=lambda x: x[0]), fmt="%s")
    else:
        print(
            f"Data length file {tgt_file_name} already exists in {src_path}, "
