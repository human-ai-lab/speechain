...
def parse():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The type of your target acoustic features.",
    )
    parser.add_argument(
        "--feat_config",
        type=str,
...
def extract_and_save_feat(
    data_index: str, wav_path: str, feat_type: str, feat_config: Dict, feat_path: str
):
    ...
    if not os.path.exists(feat_file_path):
        ...
        if feat_type == "stft":
            ...
            raise ValueError(
                f"Unknown acoustic feature: {feat_type}! It should be one of ['stft', 'logmel', 'mfcc']."
            )
...
def main(idx2wav: str, feat_type: str, feat_config: str, feat_path: str, ncpu: int):
    ...
    if not os.path.exists(os.path.join(feat_path, "idx2feat")):
        ...
        with Pool(ncpu) as executor:
            extract_and_save_feat_func = partial(
                extract_and_save_feat,
                feat_type=feat_type,
        ...
        np.savetxt(
            os.path.join(feat_path, "idx2feat"),
            sorted(idx2feat_list, key=lambda x: x[0]),
            fmt="%s",
        )

    else:
        print(
            f"Acoustic features have already existed in {feat_path}, so the extraction process is skipped."
        )
