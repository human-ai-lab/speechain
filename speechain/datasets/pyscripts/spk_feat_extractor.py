...
def main(
    ...
    spk2aver_spk_feat_path = os.path.join(
        result_path, f"spk2aver_{spk_emb_model}_spk_feat"
    )
    # skip if idx2spk_feat and spk2aver_spk_feat has already existed
    if not os.path.exists(idx2spk_feat_path) or not os.path.exists(
        spk2aver_spk_feat_path
    ):
        ...
        idx2spk = load_idx2data_file(
            os.path.join(os.path.dirname(src_idx2wav_path), "idx2spk")
        )
        ...
        device_list = (
            get_idle_gpu(ngpu, id_only=True) if ngpu > 0 else [-1 for _ in range(ncpu)]
        )
        n_proc = len(device_list) if ngpu > 0 else ncpu
        spk2wav_list = list(spk2wav_dict.items())
        func_args = [
            [dict(spk2wav_list[i::n_proc]), device_list[i]] for i in range(n_proc)
        ]
        ...
        np.savetxt(
            idx2spk_feat_path,
            sorted(idx2spk_feat.items(), key=lambda x: x[0]),
            fmt="%s",
        )
        # record the address of the .npy file of each vector
        np.savetxt(
            spk2aver_spk_feat_path,
            sorted(spk2aver_spk_feat.items(), key=lambda x: x[0]),
            fmt="%s",
        )
