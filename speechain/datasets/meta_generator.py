...
class SpeechTextMetaGenerator(ABC):
    ...
    def parse(self):
        ...
        parser = argparse.ArgumentParser(
            description="Parameters for Statistical Information Generation"
        )
        group = parser.add_argument_group("General Arguments")
        group.add_argument(
            "--src_path", type=str, default=None, help="Path to the original dataset."
        )
        group.add_argument(
            "--tgt_path",
            type=str,
            required=True,
            help="Destination path for metadata files.",
        )
        group.add_argument(
            "--txt_format",
            type=str,
            ...
            "before saving into 'idx2text'. Default is 'no-punc'",
        )
    ...
    def generate_meta_dict(
        self, src_path: str, txt_format: str, **kwargs
    ...
    def main(self):
        ...
        for subset in meta_dict.keys():
            assert (
                "idx2wav" in meta_dict[subset].keys()
                and f"idx2{txt_format}_text" in meta_dict[subset].keys()
            ...
            print(
                f"Saving metadata files {list(meta_dict[subset].keys())} of subset {subset} to {subset_path}/"
            )
