...
def main(
    ...
    if token_type == "mfa":
        idx2text_len = load_idx2data_file(
            os.path.join(
                dump_path, dataset, "data", "mfa", mfa_model, subset, "idx2text_len"
            ),
            data_type=int,
        )
    else:
    ...
    hist_plotter.plot(
        ax=ax,
        material=list(idx2text_len.values()),
    ...
    f2t_ratio = [
        idx2feat_len[idx] / idx2text_len[idx]
        for idx in idx2feat_len.keys()
        if idx in idx2text_len.keys()
    ]
    min_f2t, mean_f2t, max_f2t = (
        min(f2t_ratio),
        sum(f2t_ratio) / len(f2t_ratio),
        max(f2t_ratio),
    )
    ...
    tokenizer = (
        f"{token_type}-{token_num}" if token_type != "mfa" else f"mfa-{mfa_model}"
    )
    plt.savefig(
        os.path.join(
            plot_path,
