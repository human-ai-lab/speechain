...
def parse():
    ...
    parser.add_argument(
        "--sum_file_name", type=str, default=None, help="The name of the summary file."
    )
...
def main(src_folder: str, sum_file_name: str = None, tgt_path: str = None):
    ...
    np.savetxt(
        os.path.join(tgt_path, sum_file_name),
        sorted(file_path_summary.items(), key=lambda x: x[0]),
        fmt="%s",
    )
