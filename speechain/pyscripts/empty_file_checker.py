...
def get_empty_file(wav_and_len_list: List[List[str or int]]):
    ...
    for wav_path, wav_len in tqdm(wav_and_len_list):
        ...
        if wav.size(0) != wav_len:
            ...
            print(
                f"Find an audio file with incorrect length (should be {wav_len}, but be {wav.size(0)}): {wav_path}"
            )
...
def main(src_folder: str, vocoder: str = "hifigan", ncpu: int = 16):
    ...
    idx2wav, idx2wav_len = load_idx2data_file(
        os.path.join(src_folder, f"idx2{vocoder}_wav")
    ), load_idx2data_file(
...
if __name__ == "__main__":
    ...
    main(
        src_folder="recipes/offline_tts2asr/tts_syn_speech/librispeech/train-clean-360/return_sr=16000/seed=0_ngpu=2_batch-len=2000_spk-emb=libritts-train-clean-100-xvector-aver_model=recipes%tts%libritts%train-clean-100%exp%22.05khz_xvector_mfa_fastspeech2"
    )
