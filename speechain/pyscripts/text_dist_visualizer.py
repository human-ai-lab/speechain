...
def main(
    ...
    source_freq = [
        source_token2freq[token] if token in source_token2freq.keys() else 0
        for token in tokenizer.idx2token.keys()
    ]
    source_freq = np.array(source_freq) / max(source_freq)
    source_words = set(source_words)

    target_text_path = os.path.join(
        dump_path,
        target_dataset,
        "data",
        "wav",
    ...
    for text in target_text.values():
        target_tokens += tokenizer.text2tensor(
            text, no_sos=True, no_eos=True, return_tensor=False
        )
        target_words += text2word_list(text)
    # collect the occurrence frequency of each token and word
    target_token2freq = Counter(target_tokens)
    target_freq = [
        target_token2freq[token] if token in target_token2freq.keys() else 0
        for token in tokenizer.idx2token.keys()
    ]
    ...
    print(
        f"Total {len(target_words)} in {target_subset} of {target_dataset}: "
    ...
    print(
        f"MSE between normalized token-frequency distribution ({txt_format}-{token_type}-{token_num}) between {source_subset} of {source_dataset} and {target_subset} of {target_dataset}: "
    ...
    hist_plotter.plot(
        ax=ax,
        material=source_tokens,
        fig_name=f"{source_subset} of {source_dataset}",
        xlabel="token ids",
        ylabel="frequency",
    )
    ...
    hist_plotter.plot(
        ...
        fig_name=f"{target_subset} of {target_dataset}",
        xlabel="token ids",
        ylabel="frequency",
    )
    ...
    plt.savefig(
        os.path.join(
            plot_path,
            f"{txt_format}-{token_type}-{token_num}_{source_dataset}-{source_subset}_vs_{target_dataset}-{target_subset}",
        )
    )
    plt.close(fig=fig)
