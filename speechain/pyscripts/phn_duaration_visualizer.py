...
def main(
    ...
    if not os.path.exists(idx2wav_len_path):
        idx2wav_len_path = os.path.join(
            dump_path, dataset, "data", "wav", subset, "idx2wav_len"
        )
    ...
    for idx in tqdm(idx2wav_len.keys()):
        ...
        space_duration = [
            duration[i]
            for i, phn in enumerate(text)
            if phn == "<space>" and i not in [0, len(text) - 1]
        ]
    ...
    hist_plotter.plot(
        ax=ax,
        material=space_duration_list,
        fig_name="duration distribution of all the <space> token",
        xlabel="Num of frames "
        f"(min={min(space_duration_list)}, "
        f"mean={int(sum(space_duration_list) / len(space_duration_list))}, "
        f"max={max(space_duration_list)})",
        ylabel="frequency",
    )
    ...
    plt.savefig(
        os.path.join(
            plot_path,
            f"duration-dist_mfa={mfa_model}_data={dataset}-{subset}_sr={sample_rate}_hop={hop_len}",
        )
    )
    plt.close(fig=fig)
