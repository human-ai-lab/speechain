...
class HiFiGAN(nn.Module):
    def __init__(self, 
        ...
        ups_in_channels = [upsample_initial_channel//(2**i) for i in range(len(upsample_rates))]
        ups_out_channels = [upsample_initial_channel//(2**(i+1)) for i in range(len(upsample_rates))]
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            self.upsamples.append(
                nn.ConvTranspose1d(ups_in_channels[i], 
                                 ups_out_channels[i],
    ...
    def from_hparams(cls, source, savedir=None, run_opts=None):
        ...
        if savedir:
            ...
            if os.path.exists(weights_path):
                model.load_state_dict(torch.load(weights_path, map_location=run_opts["device"]))
