...
class TestSpeakerEncoder(unittest.TestCase):
    ...
    def test_encode_batch(self):
        ...
        self.assertEqual(out.shape, (batch_size, 192))
        self.assertTrue(torch.allclose(torch.norm(out, p=2, dim=1), torch.ones(batch_size)))
    ...
    def test_from_hparams_ecapa(self):
        model = SpeakerEncoder.from_hparams(source="ecapa", run_opts={"device": self.device})
    ...
    def test_from_hparams_xvector(self):
        model = SpeakerEncoder.from_hparams(source="xvector", run_opts={"device": self.device})
