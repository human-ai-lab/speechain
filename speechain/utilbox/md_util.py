...
def save_md_report(
    ...
    md_report = (
        f"# Overall {metric_name} Result: (mean ± std)\n"
    ...
    idx2metric_list = sorted(
        metric_results.items(), key=lambda x: x[1], reverse=desec_sort
    )[:topn_num]
    ...
    np.savetxt(
        os.path.join(
            save_path,
            f'{f"{extra_name}_" if extra_name is not None else ""}{metric_name}_results.md',
        ),
        [md_report],
        fmt="%s",
    )
